<?php
namespace App\DB;
use function commonfx\debug_log;
class Database {
    ...
    public function count(){
        $countQuery = "select count(*) as total from `{$this->table_name}` where 1";
...
//exit;
    ...
    public function get_all_paged ($start=0,$count=20){
        if($this->table_name == 'products'){
$sql = "
SELECT `products`.*,`pimages`.`imagename`
FROM `products`
LEFT JOIN  pimages ON products.id = pimages.pid
...
$sql = "SELECT products.*,pimages.imagename
FROM `products`
LEFT JOIN `pimages` ON `products`.id = `pimages`.pid
